...
int main(){
    ...
    for ( w = 0; w < 10;w++)
        ...
        for ( s = 0; s < 6; s++)
        {
            ShortestPath_Dijkstra(G, s, &P, &D);
            for (int i = 1; i < G.vexnum; i++)
            {
                printf("V%d - V%d的最短路径中的村子有：", s, i);
                printf(" V%d", i);
                int j = i;
                //由于每一段最短路径上都记录着经过的村子，所以采用嵌套的方式输出即可得到各个最短路径上的所有顶点
                while (P[j] != 0)
                {
                    printf(" V%d", P[j]);
                    j = P[j];
            ...
            {
                W[s] += D[i];
                printf("V%d - V%d : %d \n", G.vexs[s], G.vexs[i], D[i]);
            }
            printf("在V%d建医院到各村庄最短路径长度和为:%d\n", s, W[s]);
        }
        for (int i = 0; i < 6;i++)
        {
            int a = 0;
